...
public class ScoreDetailsValidatorITCL extends AbstractValidationRule<ScoreDetails> {
	...
	public OperationResult.StepResult apply(ScoreDetails score) {
		...
		List<String> keysToValidate = JSONUtils.parse(metaDataService.fetchByValue("validation", "score_float").getDomainValues().toString(), ArrayList.class);
		var extdVal = score.getExtendedAttributes();
		try {
			extdVal.fieldNames().forEachRemaining(fieldName -> {
				if (keysToValidate.contains(fieldName) && (extdVal.get(fieldName).asText().isEmpty() || !(extdVal.get(fieldName).asText().matches("^-?[0-9]\\d*(\\.\\d+)?$"))))
					throw new IllegalArgumentException(StringUtils.format("Value should be in Number and can't be empty, Kindly check value of Params => " + fieldName + " : {}", extdVal.get(fieldName)));
			});
		} catch (Exception e) {
	...
	public String rcsidCheck(ScoreDetails score) {
		...
		String errorstr = "";
		OutletDetails outletObj = outletService.findByOutletCode(score.getOutletCode());
		if (outletObj == null) {
		...
		if (!errorstr.equals("")) return errorstr;
	...
	public String dateErrorCheck(ScoreDetails score, List<String> errors) {
      ...
      Logger logger = LoggerFactory.getLogger(this.getClass());
		/*
		 * Validation for Loyalty score data can't be uploaded for current month and
		 * future month and Validation for Loyalty score data can't be upload for
		 * previous month in first week of month
		 */
		if (score.getFeature().equalsIgnoreCase("loyalty")) {
			String errorstr = "";
//			try {
//
//				List<String> blockDate = Arrays.asList("1,2,3,4,5,6,7".split(","));
//				Calendar cal1 = Calendar.getInstance();
//				cal1.setTime(new Date());
//
//				String date = cal1.get(Calendar.DATE) + "";
//				if (blockDate.indexOf(date) > -1) {
//					throw new IllegalArgumentException("Cannot upload on this date ");
//				}
//
//			} catch (Exception e) {
//				return e.getMessage();
//			}
			...
			/*
			 * There should be no pending status in db against any user for any month
			 */
