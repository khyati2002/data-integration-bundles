...
public class SimaCustomerGrouping extends AbstractTransformer<Map<String,Object>,Map<String,Object>> {
    ...
    public Object transform(Map<String, Object> stringObjectMap) {
        ...
        JsonNode customerGroupObject = JSONUtils.toJsonNode(customerGroup).get(0);
        ...
        if(isValidCondition(customerGroupObject)){
            ...
            throw new TransformationException("OutletSelection and SelectionCriteria keys not found!");
        }
    ...
    private Map<String, List<String>> createObjectOfFilters(JsonNode selectionObject, String key2){
        ...
        while (true) {
            ...
            if(mapOfCodeAndValues.containsKey(key2)) {
                List<GenericEntity> codeAndValueMapping = genericEntityRepository.findByNameAndKey1AndKey2("Outletcategory", mapOfCodeAndValues.get(key2), codeValue);
                if(codeAndValueMapping.isEmpty()){
                    throw new TransformationException("{} code not found in outlet mapping for key {}", codeValue, mapOfCodeAndValues.get(key2));
                }
    ...
    private void createOutletCodeObjectWithSelectionCriteria(JsonNode customerGroupObject,Map<String, String> outletCodeObject,List<String> listOfAndOperators,List<String> listOfOrOperators,HashMap<String, Map<String, List<String>>> objectForQuery){
        ...
        for(int i = 0; i < selectionCriteria.size(); i++){
            ...
            List<GenericEntity> customerGroupMapping = genericEntityRepository.findByNameAndKey1("customerGroupMapping", fieldNumber);
        ...
        String selectStatement = "select outletcode from ck_outlet_details where ";
        String finalQuery = selectStatement + queryAndOperator + (queryOrOperator.isEmpty()? " " : " or ") + queryOrOperator + ";";
        List<Map<String, String>> outletCodes = (List<Map<String, String>>) EntityUtils.get().findDataByQuery(Map.class, finalQuery, true);
        for(int i = 0; i< outletCodes.size(); i++){
            String outletCode = outletCodes.get(i).get("outletcode");
            outletCodeObject.put(outletCode, outletCode);
        }
    }
